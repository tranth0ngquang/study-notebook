...
-- Run this file once against the target Supabase project in the SQL Editor.
...
alter table public.lectures
  drop constraint if exists lectures_duration_minutes_check;

alter table public.lectures
  add constraint lectures_duration_minutes_check
  check (duration_minutes is null or duration_minutes > 0);

alter table public.lectures
  drop constraint if exists lectures_lecture_number_check;

alter table public.lectures
  add constraint lectures_lecture_number_check
  check (lecture_number is null or lecture_number > 0);
